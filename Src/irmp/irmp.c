 ...
 */
uint_fast8_t
irmp_get_data( IRMP_DATA *irmp_data_p, IRMP_STATE *state) {
    uint_fast8_t rtc = FALSE;

    if (state->irmp_ir_detected) {
        switch (state->irmp_protocol) {
#if IRMP_SUPPORT_SAMSUNG_PROTOCOL == 1
            case IRMP_SAMSUNG_PROTOCOL:
                if ((state->irmp_command >> 8) == (~state->irmp_command & 0x00FF))
                {
                    state->irmp_command &= 0xff;
                    state->irmp_command |= state->irmp_id << 8;
                    rtc = TRUE;
                }
                break;
            ...
            case IRMP_NEC_PROTOCOL:
                ...
                else if (state->irmp_address == 0x87EE)
                {
#ifdef ANALYZE
                    ANALYZE_PRINTF ("Switching to APPLE protocol\n");
#endif // ANALYZE
                    state->irmp_protocol = IRMP_APPLE_PROTOCOL;
                    state->irmp_address = (state->irmp_command & 0xFF00) >> 8;
                    state->irmp_command &= 0x00FF;
                    rtc = TRUE;
                }
                break;
#endif
            ...
            case IRMP_RUWIDO_PROTOCOL:
                if (((state->irmp_command >> 1) & 0x0001) == (~state->irmp_command & 0x0001))
                {
                    state->irmp_command >>= 1;
                    rtc = TRUE;
                }
                break;
#endif
            ...
            case IRMP_RC5_PROTOCOL:
                state->irmp_address &= ~0x20;                              // clear toggle bit
                rtc = TRUE;
                break;
#endif
#if IRMP_SUPPORT_S100_PROTOCOL == 1
            ...
            case IRMP_IR60_PROTOCOL:
                if (state->irmp_command != 0x007d)                         // 0x007d (== 62<<1 + 1) is start instruction frame
                {
                    rtc = TRUE;
                }
                else
        ...
        if (rtc) {
            irmp_data_p->protocol = state->irmp_protocol;
            irmp_data_p->address = state->irmp_address;
            irmp_data_p->command = state->irmp_command;
            irmp_data_p->flags = state->irmp_flags;
        } else {
            state->irmp_protocol = IRMP_UNKNOWN_PROTOCOL;
        }

        state->irmp_command = 0;                                      // don't reset irmp_protocol here, needed for detection of NEC & JVC repetition frames!
        state->irmp_address = 0;
        state->irmp_flags = 0;

        state->irmp_ir_detected = FALSE;
    }
...
static void
irmp_store_bit(uint_fast8_t value, IRMP_STATE* state) {
#if IRMP_SUPPORT_ACP24_PROTOCOL == 1
    if (irmp_param.protocol == IRMP_ACP24_PROTOCOL)                                                 // squeeze 64 bits into 16 bits:
    {
        if (value)
        {
            ...
            switch (irmp_bit)
            {
                case  0: state->irmp_tmp_command |= (1<<15); break;                                        // N
                case  2: state->irmp_tmp_command |= (1<<13); break;                                        // V
                case  3: state->irmp_tmp_command |= (1<<12); break;                                        // V
                case  4: state->irmp_tmp_command |= (1<<10); break;                                        // M
                case  5: state->irmp_tmp_command |= (1<< 9); break;                                        // M
                case  6: state->irmp_tmp_command |= (1<< 8); break;                                        // M
                case 20: state->irmp_tmp_command |= (1<< 6); break;                                        // t
                case 22: state->irmp_tmp_command |= (1<<11); break;                                        // v
                case 23: state->irmp_tmp_command |= (1<< 7); break;                                        // m
                case 24: state->irmp_tmp_command |= (1<<14); break;                                        // A
                case 26: state->irmp_tmp_command |= (1<< 5); break;                                        // x
                case 44: state->irmp_tmp_command |= (1<< 4); break;                                        // y
                case 66: state->irmp_tmp_command |= (1<< 3); break;                                        // T
                case 67: state->irmp_tmp_command |= (1<< 2); break;                                        // T
                case 68: state->irmp_tmp_command |= (1<< 1); break;                                        // T
                case 69: state->irmp_tmp_command |= (1<< 0); break;                                        // T
            }
        }
    }
    else
...
#if IRMP_SUPPORT_GRUNDIG_NOKIA_IR60_PROTOCOL == 1
    if (state->irmp_bit == 0 && state->irmp_param.protocol == IRMP_GRUNDIG_PROTOCOL)
    {
        first_bit = value;
    }
    else
    ...
    if (state->irmp_bit >= state->irmp_param.address_offset && state->irmp_bit < state->irmp_param.address_end) {
        if (state->irmp_param.lsb_first) {
            state->irmp_tmp_address |= (((uint_fast16_t) (value))
                    << (state->irmp_bit - state->irmp_param.address_offset));   // CV wants cast
        } else {
            state->irmp_tmp_address <<= 1;
            state->irmp_tmp_address |= value;
        }
    } else if (state->irmp_bit >= state->irmp_param.command_offset && state->irmp_bit < state->irmp_param.command_end) {
        if (state->irmp_param.lsb_first) {
#if IRMP_SUPPORT_SAMSUNG48_PROTOCOL == 1
            if (irmp_param.protocol == IRMP_SAMSUNG48_PROTOCOL && irmp_bit >= 32)
            {
                state->irmp_tmp_id |= (((uint_fast16_t) (value)) << (irmp_bit - 32));   // CV wants cast
            }
            else
#endif
            {
                state->irmp_tmp_command |= (((uint_fast16_t) (value))
                        << (state->irmp_bit - state->irmp_param.command_offset));   // CV wants cast
            }
        } else {
            state->irmp_tmp_command <<= 1;
            state->irmp_tmp_command |= value;
        }
    }
...
#if IRMP_SUPPORT_NEC42_PROTOCOL == 1
    if (state->irmp_param.protocol == IRMP_NEC42_PROTOCOL && state->irmp_bit >= 13 && state->irmp_bit < 26)
    {
        state->irmp_tmp_address2 |= (((uint_fast16_t) (value)) << (state->irmp_bit - 13));                             // CV wants cast
    }
    else
#endif

#if IRMP_SUPPORT_SAMSUNG_PROTOCOL == 1
    if (state->irmp_param.protocol == IRMP_SAMSUNG_PROTOCOL && state->irmp_bit >= SAMSUNG_ID_OFFSET && state->irmp_bit < SAMSUNG_ID_OFFSET + SAMSUNG_ID_LEN)
    {
        state->irmp_tmp_id |= (((uint_fast16_t) (value)) << (state->irmp_bit - SAMSUNG_ID_OFFSET));                    // store with LSB first
    }
    else
#endif

#if IRMP_SUPPORT_KASEIKYO_PROTOCOL == 1
    if (state->irmp_param.protocol == IRMP_KASEIKYO_PROTOCOL)
    {
        if (state->irmp_bit >= 20 && state->irmp_bit < 24)
        {
            state->irmp_tmp_command |= (((uint_fast16_t) (value)) << (state->irmp_bit - 8));      // store 4 system bits (genre 1) in upper nibble with LSB first
        }
        else if (state->irmp_bit >= 24 && state->irmp_bit < 28)
        {
            state->genre2 |= (((uint_fast8_t) (value)) << (state->irmp_bit - 20));                // store 4 system bits (genre 2) in upper nibble with LSB first
        }

        if (state->irmp_bit < KASEIKYO_COMPLETE_DATA_LEN)
        {
            if (value)
            {
                state->xor_check[state->irmp_bit / 8] |= 1 << (state->irmp_bit % 8);
            }
            else
            {
                state->xor_check[state->irmp_bit / 8] &= ~(1 << (state->irmp_bit % 8));
    ...
    {
        if (irmp_bit == 72 )
        {                                                                           // state->irmp_tmp_address, state->irmp_tmp_command received: check parity & compress
            mitsu_parity = PARITY_CHECK_OK;

            check = state->irmp_tmp_address >> 8;                                          // inverted upper byte == lower byte?
            check = ~ check;

            if (check == (state->irmp_tmp_address & 0xFF))
            {                                                                       // ok:
                state->irmp_tmp_address <<= 8;                                             // throw away upper byte
            }
            else
            {
            ...
            check = state->irmp_tmp_command >> 8;                                          // inverted upper byte == lower byte?
            check = ~ check;
            if (check == (state->irmp_tmp_command & 0xFF))
            {                                                                       // ok:  pack together
                state->irmp_tmp_address |= state->irmp_tmp_command & 0xFF;                        // byte 1, byte2 in state->irmp_tmp_address, state->irmp_tmp_command can be used for byte 3
            }
            else
            {
                mitsu_parity = PARITY_CHECK_FAILED;
            }
            state->irmp_tmp_command = 0;
        }

        if (irmp_bit >= 72 )
        {                                                                           // receive 3. word in state->irmp_tmp_command
            state->irmp_tmp_command <<= 1;
            state->irmp_tmp_command |= value;
        }
    }
...
irmp_store_bit2 (uint_fast8_t value)
{
    ...
    {
        state->irmp_tmp_address2 |= (((uint_fast16_t) (value)) << (irmp_bit2 - irmp_param2.address_offset));   // CV wants cast
    }
    else if (irmp_bit2 >= irmp_param2.command_offset && irmp_bit2 < irmp_param2.command_end)
    {
        state->irmp_tmp_command2 |= (((uint_fast16_t) (value)) << (irmp_bit2 - irmp_param2.command_offset));   // CV wants cast
    }
}
 ...
 */
uint_fast8_t
irmp_ISR(GPIO_TypeDef *port, uint16_t pin,  IRMP_STATE* state) {
    ...
    irmp_input = HAL_GPIO_ReadPin(port, pin);
    ...
    if (!state->irmp_ir_detected)                                                     // ir code already detected?
    {                                                                           // no...
...
//              irmp_busy_flag = TRUE;
                ...
                {                                                               // set flags for counting the time of darkness...
                    ...
                    state->irmp_tmp_command2       = 0;
                    state->irmp_tmp_address2       = 0;
#endif
#if IRMP_SUPPORT_LGAIR_PROTOCOL == 1
            ...
            {                                                                   // ...and are counting the time of darkness
                ...
                {                                                               // yes
                    ...
                    {                                                           // yes...
                        ...
                        if (state->irmp_protocol == IRMP_JVC_PROTOCOL)                 // don't show eror if JVC protocol, irmp_pulse_time has been set below!
                        {
                            ;
                        }
                        else
...
#if IRMP_SUPPORT_JVC_PROTOCOL == 1
                    if (state->irmp_protocol == IRMP_JVC_PROTOCOL &&                                                       // last protocol was JVC, awaiting repeat frame
                    ...
                    if (irmp_start_bit_detected) {
                        memcpy_P(&state->irmp_param, irmp_param_p, sizeof(IRMP_PARAMETER));

                        if (!(state->irmp_param.flags & IRMP_PARAM_FLAG_IS_MANCHESTER)) {
#ifdef ANALYZE
...
#if IRMP_SUPPORT_RC6_PROTOCOL == 1
                        if (state->irmp_param.protocol == IRMP_RC6_PROTOCOL)
                        {
#ifdef ANALYZE
...
#if IRMP_SUPPORT_MANCHESTER == 1
                    if ((state->irmp_param.flags & IRMP_PARAM_FLAG_IS_MANCHESTER) &&
                            state->irmp_param.protocol != IRMP_RUWIDO_PROTOCOL && // Manchester, but not RUWIDO
                                                                                  state->irmp_param.protocol != IRMP_RC6_PROTOCOL)      // Manchester, but not RC6
                    {
                        if (irmp_pause_time > state->irmp_param.pulse_1_len_max && irmp_pause_time <= 2 * state->irmp_param.pulse_1_len_max)
                        {
#ifdef ANALYZE
                            ...
                            ANALYZE_NEWLINE ();
#endif // ANALYZE
                            irmp_store_bit ((state->irmp_param.flags & IRMP_PARAM_FLAG_1ST_PULSE_IS_1) ? 0 : 1, state);
                        }
                        else if (! last_value)  // && irmp_pause_time >= irmp_param.pause_1_len_min && irmp_pause_time <= irmp_param.pause_1_len_max)
                        {
                            ...
                            ANALYZE_NEWLINE ();
#endif // ANALYZE
                            irmp_store_bit ((state->irmp_param.flags & IRMP_PARAM_FLAG_1ST_PULSE_IS_1) ? 1 : 0, state);
                        }
                    }
                    else
...
#if IRMP_SUPPORT_DENON_PROTOCOL == 1
                    if (state->irmp_param.protocol == IRMP_DENON_PROTOCOL)
                    {
#ifdef ANALYZE
                        ANALYZE_PRINTF ("%8.3fms [bit %2d: pulse = %3d, pause = %3d] ", (double) (time_counter * 1000) / F_INTERRUPTS, irmp_bit, irmp_pulse_time, irmp_pause_time);
#endif // ANALYZE
            ...
            {                                                                   // counting the time of darkness....
                ...
                if (irmp_input)                                                 // still dark?
                {                                                               // yes...
                    if (state->irmp_bit == state->irmp_param.complete_len && state->irmp_param.stop_bit == 1) {
                        if (
#if IRMP_SUPPORT_MANCHESTER == 1
                                (state->irmp_param.flags & IRMP_PARAM_FLAG_IS_MANCHESTER) ||
#endif
#if IRMP_SUPPORT_SERIAL == 1
                                (irmp_param.flags & IRMP_PARAM_FLAG_IS_SERIAL) ||
#endif
                                (irmp_pulse_time >= state->irmp_param.pulse_0_len_min &&
                                 irmp_pulse_time <= state->irmp_param.pulse_0_len_max)) {
#ifdef ANALYZE
...
#if IRMP_SUPPORT_SIRCS_PROTOCOL == 1
                        if (state->irmp_param.protocol == IRMP_SIRCS_PROTOCOL &&                           // Sony has a variable number of bits:
                            irmp_pause_time > SIRCS_PAUSE_LEN_MAX &&                                // minimum is 12
                                                                                                    state->irmp_bit >= 12 - 1)                                                     // pause too long?
                        {                                                                           // yes, break and close this frame
                            state->irmp_param.complete_len = state->irmp_bit + 1;                                 // set new complete length
                            got_light = TRUE;                                                       // this is a lie, but helps (generates stop bit)
                            state->irmp_tmp_address |= (state->irmp_bit - SIRCS_MINIMUM_DATA_LEN + 1) << 8;       // new: store number of additional bits in upper byte of address!
                            state->irmp_param.command_end = state->irmp_param.command_offset + state->irmp_bit + 1;      // correct command length
...
#endif
#if IRMP_SUPPORT_GRUNDIG_NOKIA_IR60_PROTOCOL == 1
                        if (state->irmp_param.protocol == IRMP_GRUNDIG_PROTOCOL && !state->irmp_param.stop_bit)
                        {
                            if (irmp_pause_time > IR60_TIMEOUT_LEN && (state->irmp_bit == 5 || state->irmp_bit == 6))
                            {
#ifdef ANALYZE
                                ANALYZE_PRINTF ("Switching to IR60 protocol\n");
#endif // ANALYZE
                                got_light = TRUE;                                       // this is a lie, but generates a stop bit ;-)
                                state->irmp_param.stop_bit = TRUE;                             // set flag

                                state->irmp_param.protocol         = IRMP_IR60_PROTOCOL;       // change protocol
                                state->irmp_param.complete_len     = IR60_COMPLETE_DATA_LEN;   // correct complete len
                                state->irmp_param.address_offset   = IR60_ADDRESS_OFFSET;
                                state->irmp_param.address_end      = IR60_ADDRESS_OFFSET + IR60_ADDRESS_LEN;
                                state->irmp_param.command_offset   = IR60_COMMAND_OFFSET;
                                state->irmp_param.command_end      = IR60_COMMAND_OFFSET + IR60_COMMAND_LEN;

                                state->irmp_tmp_command <<= 1;
                                state->irmp_tmp_command |= first_bit;
                            }
                            else if (irmp_pause_time >= 2 * state->irmp_param.pause_1_len_max && state->irmp_bit >= GRUNDIG_COMPLETE_DATA_LEN - 2)
                            {                                                           // special manchester decoder
                                state->irmp_param.complete_len = GRUNDIG_COMPLETE_DATA_LEN;    // correct complete len
                                got_light = TRUE;                                       // this is a lie, but generates a stop bit ;-)
                                state->irmp_param.stop_bit = TRUE;                             // set flag
                            }
                            else if (state->irmp_bit >= GRUNDIG_COMPLETE_DATA_LEN)
                            {
#ifdef ANALYZE
                                ANALYZE_PRINTF ("Switching to NOKIA protocol, irmp_bit = %d\n", irmp_bit);
#endif // ANALYZE
                                state->irmp_param.protocol         = IRMP_NOKIA_PROTOCOL;      // change protocol
                                state->irmp_param.address_offset   = NOKIA_ADDRESS_OFFSET;
                                state->irmp_param.address_end      = NOKIA_ADDRESS_OFFSET + NOKIA_ADDRESS_LEN;
                                state->irmp_param.command_offset   = NOKIA_COMMAND_OFFSET;
                                state->irmp_param.command_end      = NOKIA_COMMAND_OFFSET + NOKIA_COMMAND_LEN;

                                if (state->irmp_tmp_command & 0x300)
                                {
                                    state->irmp_tmp_address = (state->irmp_tmp_command >> 8);
                                    state->irmp_tmp_command &= 0xFF;
                                }
                            }
                        }
                        else
#endif
#if IRMP_SUPPORT_SIEMENS_OR_RUWIDO_PROTOCOL == 1
                        if (state->irmp_param.protocol == IRMP_RUWIDO_PROTOCOL && !state->irmp_param.stop_bit)
                        {
                            if (irmp_pause_time >= 2 * state->irmp_param.pause_1_len_max && state->irmp_bit >= RUWIDO_COMPLETE_DATA_LEN - 2)
                            {                                                           // special manchester decoder
                                state->irmp_param.complete_len = RUWIDO_COMPLETE_DATA_LEN;     // correct complete len
                                got_light = TRUE;                                       // this is a lie, but generates a stop bit ;-)
                                state->irmp_param.stop_bit = TRUE;                             // set flag
                            }
                            else if (state->irmp_bit >= RUWIDO_COMPLETE_DATA_LEN)
                            {
#ifdef ANALYZE
                                ANALYZE_PRINTF ("Switching to SIEMENS protocol\n");
#endif // ANALYZE
                                state->irmp_param.protocol         = IRMP_SIEMENS_PROTOCOL;    // change protocol
                                state->irmp_param.address_offset   = SIEMENS_ADDRESS_OFFSET;
                                state->irmp_param.address_end      = SIEMENS_ADDRESS_OFFSET + SIEMENS_ADDRESS_LEN;
                                state->irmp_param.command_offset   = SIEMENS_COMMAND_OFFSET;
                                state->irmp_param.command_end      = SIEMENS_COMMAND_OFFSET + SIEMENS_COMMAND_LEN;

                                //                   76543210
                                // RUWIDO:  AAAAAAAAACCCCCCCp
                                // SIEMENS: AAAAAAAAAAACCCCCCCCCCp
                                state->irmp_tmp_address <<= 2;
                                state->irmp_tmp_address |= (state->irmp_tmp_command >> 6);
                                state->irmp_tmp_command &= 0x003F;
//                              state->irmp_tmp_command <<= 4;
                                state->irmp_tmp_command |= last_value;
                            }
                        }
                        else
#endif
...
#if IRMP_SUPPORT_MANCHESTER == 1
                        if ((state->irmp_param.flags & IRMP_PARAM_FLAG_IS_MANCHESTER) &&
                            irmp_pause_time >= 2 * state->irmp_param.pause_1_len_max && state->irmp_bit >= state->irmp_param.complete_len - 2 && !state->irmp_param.stop_bit)
                        {                                                       // special manchester decoder
                            got_light = TRUE;                                   // this is a lie, but generates a stop bit ;-)
                            state->irmp_param.stop_bit = TRUE;                         // set flag
                        }
                        ...
                        if (irmp_pause_time > IRMP_TIMEOUT_LEN)                 // timeout?
                        {                                                       // yes...
                            if (state->irmp_bit == state->irmp_param.complete_len - 1 && state->irmp_param.stop_bit == 0) {
                                state->irmp_bit++;
                            }
#if IRMP_SUPPORT_NEC_PROTOCOL == 1
                                else if ((state->irmp_param.protocol == IRMP_NEC_PROTOCOL || state->irmp_param.protocol == IRMP_NEC42_PROTOCOL) && state->irmp_bit == 0)
                                {                                                               // it was a non-standard repetition frame
                                    ...
                                    {
                                        state->irmp_param.stop_bit     = TRUE;                         // set flag
                                        state->irmp_param.protocol     = IRMP_NEC_PROTOCOL;            // switch protocol
                                        state->irmp_param.complete_len = state->irmp_bit;                     // patch length: 16 or 17
                                        state->irmp_tmp_address = last_irmp_address;                   // address is last address
                                        state->irmp_tmp_command = last_irmp_command;                   // command is last command
                                        state->irmp_flags |= IRMP_FLAG_REPETITION;
...
#endif // IRMP_SUPPORT_NEC_PROTOCOL == 1
#if IRMP_SUPPORT_JVC_PROTOCOL == 1
                                else if (state->irmp_param.protocol == IRMP_NEC_PROTOCOL && (state->irmp_bit == 16 || state->irmp_bit == 17))      // it was a JVC stop bit
                                {
#ifdef ANALYZE
                                    ANALYZE_PRINTF ("Switching to JVC protocol, irmp_bit = %d\n", irmp_bit);
#endif // ANALYZE
                                state->irmp_param.stop_bit     = TRUE;                                     // set flag
                                state->irmp_param.protocol     = IRMP_JVC_PROTOCOL;                        // switch protocol
                                state->irmp_param.complete_len = state->irmp_bit;                                 // patch length: 16 or 17
                                    state->irmp_tmp_command        = (state->irmp_tmp_address >> 4);                  // set command: upper 12 bits are command bits
                                    state->irmp_tmp_address        = state->irmp_tmp_address & 0x000F;                // lower 4 bits are address bits
                                ...
                                else if (irmp_param.protocol == IRMP_NEC_PROTOCOL && (irmp_bit == 28 || irmp_bit == 29))      // it was a LGAIR stop bit
                                {
                                    ...
                                    state->irmp_tmp_command        = irmp_lgair_command;                       // set command: upper 8 bits are command bits
                                    state->irmp_tmp_address        = irmp_lgair_address;                       // lower 4 bits are address bits
                                    irmp_start_bit_detected = 1;                                        // tricky: don't wait for another start bit...
                                }
...
#if IRMP_SUPPORT_NEC42_PROTOCOL == 1
#if IRMP_SUPPORT_NEC_PROTOCOL == 1
                                else if (state->irmp_param.protocol == IRMP_NEC42_PROTOCOL && state->irmp_bit == 32)      // it was a NEC stop bit
                                {
#ifdef ANALYZE
                                ...
                                state->irmp_param.complete_len = state->irmp_bit;                                 // patch length: 16 or 17
                                    ...
                                    state->irmp_tmp_address        |= (state->irmp_tmp_address2 & 0x0007) << 13;      // fm 2012-02-13: 12 -> 13
                                    state->irmp_tmp_command        = (state->irmp_tmp_address2 >> 3) | (state->irmp_tmp_command << 10);
                                }
#endif // IRMP_SUPPORT_NEC_PROTOCOL == 1
#if IRMP_SUPPORT_LGAIR_PROTOCOL == 1
                                else if (irmp_param.protocol == IRMP_NEC42_PROTOCOL && irmp_bit == 28)      // it was a NEC stop bit
                                {
                                    ...
                                    irmp_param.protocol     = IRMP_LGAIR_PROTOCOL;                      // switch protocol
                                    irmp_param.complete_len = irmp_bit;                                 // patch length: 16 or 17
                                    state->irmp_tmp_address        = irmp_lgair_address;
                                    state->irmp_tmp_command        = irmp_lgair_command;
                                }
#endif // IRMP_SUPPORT_LGAIR_PROTOCOL == 1
#if IRMP_SUPPORT_JVC_PROTOCOL == 1
                                else if (state->irmp_param.protocol == IRMP_NEC42_PROTOCOL && (state->irmp_bit == 16 || state->irmp_bit == 17))  // it was a JVC stop bit
                                {
#ifdef ANALYZE
                                    ANALYZE_PRINTF ("Switching to JVC protocol, irmp_bit = %d\n", irmp_bit);
#endif // ANALYZE
                                ...
                                state->irmp_param.complete_len = state->irmp_bit;                                 // patch length: 16 or 17
                                    ...
                                    state->irmp_tmp_command        = (state->irmp_tmp_address >> 4) | (state->irmp_tmp_address2 << 9);   // set command: upper 12 bits are command bits
                                    state->irmp_tmp_address        = state->irmp_tmp_address & 0x000F;                            // lower 4 bits are address bits
                                }
#endif // IRMP_SUPPORT_JVC_PROTOCOL == 1
                                ...
                                else if (irmp_param.protocol == IRMP_RCMM32_PROTOCOL && (irmp_bit == 12 || irmp_bit == 24))  // it was a RCMM stop bit
                                {
                                    if (irmp_bit == 12)
                                    {
                                        state->irmp_tmp_command = (state->irmp_tmp_address & 0xFF);                   // set command: lower 8 bits are command bits
                                        state->irmp_tmp_address >>= 8;                                         // upper 4 bits are address bits
                                ...
                                else if (irmp_param.protocol == IRMP_MATSUSHITA_PROTOCOL && irmp_bit == 22)  // it was a TECHNICS stop bit
                                {
...
#endif // ANALYZE
                                    // Situation:
                                    // The first 12 bits have been stored in state->irmp_tmp_command (LSB first)
                                    // The following 10 bits have been stored in state->irmp_tmp_address (LSB first)
                                    ...
                                    state->irmp_tmp_address <<= 1;
                                    if (state->irmp_tmp_command & (1<<11))
                                    {
                                        state->irmp_tmp_address |= 1;
                                        state->irmp_tmp_command &= ~(1<<11);
                                    }

                                    if (state->irmp_tmp_command == ((~state->irmp_tmp_address) & 0x07FF))
                                    {
                                        state->irmp_tmp_address = 0;
...
#if IRMP_SUPPORT_MANCHESTER == 1
                    if ((state->irmp_param.flags & IRMP_PARAM_FLAG_IS_MANCHESTER))                                     // Manchester
                    {
#if 1
                        if (irmp_pulse_time > state->irmp_param.pulse_1_len_max /* && irmp_pulse_time <= 2 * irmp_param.pulse_1_len_max */)
                        ...
                        {
#if IRMP_SUPPORT_RC6_PROTOCOL == 1
                            if (state->irmp_param.protocol == IRMP_RC6_PROTOCOL && state->irmp_bit == 4 && irmp_pulse_time > RC6_TOGGLE_BIT_LEN_MIN)         // RC6 toggle bit
                            {
#ifdef ANALYZE
                                ANALYZE_PUTCHAR ('T');
#endif // ANALYZE
                                if (state->irmp_param.complete_len == RC6_COMPLETE_DATA_LEN_LONG)                      // RC6 mode 6A
                                {
                                    irmp_store_bit (1, state);
                                    last_value = 1;
                                }
...
#endif // IRMP_SUPPORT_RC6_PROTOCOL == 1
                            {
                                ...
                                if (state->irmp_param.protocol == IRMP_RC6_PROTOCOL && state->irmp_bit == 4 && irmp_pulse_time > RC6_TOGGLE_BIT_LEN_MIN)      // RC6 toggle bit
                                {
                                    ...
                                    if (irmp_pause_time > 2 * state->irmp_param.pause_1_len_max)
                                    {
                                        last_value = 0;
                                    }
                                    else
...
#endif // IRMP_SUPPORT_RC6_PROTOCOL == 1
                                {
                                    ...
                                    last_value = (state->irmp_param.flags & IRMP_PARAM_FLAG_1ST_PULSE_IS_1) ? 1 : 0;
                                }
                            }
                        }
                        else if (irmp_pulse_time >= state->irmp_param.pulse_1_len_min && irmp_pulse_time <= state->irmp_param.pulse_1_len_max
                                 /* && irmp_pause_time <= 2 * irmp_param.pause_1_len_max */)
                        {
                            uint_fast8_t manchester_value;

                            if (last_pause > state->irmp_param.pause_1_len_max && last_pause <= 2 * state->irmp_param.pause_1_len_max)
                            {
...
#if IRMP_SUPPORT_RC6_PROTOCOL == 1
                            if (state->irmp_param.protocol == IRMP_RC6_PROTOCOL && state->irmp_bit == 1 && manchester_value == 1)     // RC6 mode != 0 ???
                            {
#ifdef ANALYZE
                                ANALYZE_PRINTF ("Switching to RC6A protocol\n");
#endif // ANALYZE
                                state->irmp_param.complete_len = RC6_COMPLETE_DATA_LEN_LONG;
                                state->irmp_param.address_offset = 5;
                                state->irmp_param.address_end = state->irmp_param.address_offset + 15;
                                state->irmp_param.command_offset = state->irmp_param.address_end + 1;                                 // skip 1 system bit, changes like a toggle bit
                                state->irmp_param.command_end = state->irmp_param.command_offset + 16 - 1;
                                state->irmp_tmp_address = 0;
                            }
#endif // IRMP_SUPPORT_RC6_PROTOCOL == 1

                            irmp_store_bit (manchester_value, state);
                        }
                        else
                        {
                        ...
                        if (irmp_param2.protocol == IRMP_FDC_PROTOCOL && irmp_pulse_time >= FDC_PULSE_LEN_MIN && irmp_pulse_time <= FDC_PULSE_LEN_MAX)
                        {
                            ...
                            if (! irmp_param.protocol)
                            {
                                ...
                                irmp_param2.protocol = 0;
                                state->irmp_tmp_address = state->irmp_tmp_address2;
                                state->irmp_tmp_command = state->irmp_tmp_command2;
                            }
                        }
#endif // IRMP_SUPPORT_FDC_PROTOCOL == 1
#if IRMP_SUPPORT_RC5_PROTOCOL == 1 && IRMP_SUPPORT_RCCAR_PROTOCOL == 1
...
#if IRMP_SUPPORT_SAMSUNG_PROTOCOL == 1
                    if (state->irmp_param.protocol == IRMP_SAMSUNG_PROTOCOL && state->irmp_bit == 16)       // Samsung: 16th bit
                    {
                        ...
                        else  if (irmp_pulse_time >= SAMSUNG_PULSE_LEN_MIN && irmp_pulse_time <= SAMSUNG_PULSE_LEN_MAX)
                        {
                            ...
                            ANALYZE_PRINTF ("Switching to SAMSUNG32 protocol ");
#endif // ANALYZE
                            state->irmp_param.protocol         = IRMP_SAMSUNG32_PROTOCOL;
                            state->irmp_param.command_offset   = SAMSUNG32_COMMAND_OFFSET;
                            state->irmp_param.command_end      = SAMSUNG32_COMMAND_OFFSET + SAMSUNG32_COMMAND_LEN;
                            state->irmp_param.complete_len     = SAMSUNG32_COMPLETE_DATA_LEN;
#endif
...
#if IRMP_SUPPORT_NEC42_PROTOCOL == 1
                    if (state->irmp_param.protocol == IRMP_NEC42_PROTOCOL &&
#else // IRMP_SUPPORT_NEC_PROTOCOL instead
                    if (irmp_param.protocol == IRMP_NEC_PROTOCOL &&
#endif // IRMP_SUPPORT_NEC42_PROTOCOL == 1
state->irmp_bit == 8 && irmp_pause_time >= NEC_START_BIT_PAUSE_LEN_MIN && irmp_pause_time <= NEC_START_BIT_PAUSE_LEN_MAX)
                    {
#ifdef ANALYZE
                        ANALYZE_PRINTF ("Switching to NEC16 protocol\n");
#endif // ANALYZE
                        state->irmp_param.protocol         = IRMP_NEC16_PROTOCOL;
                        state->irmp_param.address_offset   = NEC16_ADDRESS_OFFSET;
                        state->irmp_param.address_end      = NEC16_ADDRESS_OFFSET + NEC16_ADDRESS_LEN;
                        state->irmp_param.command_offset   = NEC16_COMMAND_OFFSET;
                        state->irmp_param.command_end      = NEC16_COMMAND_OFFSET + NEC16_COMMAND_LEN;
                        state->irmp_param.complete_len     = NEC16_COMPLETE_DATA_LEN;
                        wait_for_space = 0;
                    }
                    else
                    ...
                    if (irmp_pulse_time >= state->irmp_param.pulse_1_len_min &&
                        irmp_pulse_time <= state->irmp_param.pulse_1_len_max &&
                        irmp_pause_time >= state->irmp_param.pause_1_len_min && irmp_pause_time <=
                                                                                        state->irmp_param.pause_1_len_max) {                                                               // pulse & pause timings correct for "1"?
#ifdef ANALYZE
                        ANALYZE_PUTCHAR ('1');
                        ANALYZE_NEWLINE ();
#endif // ANALYZE
                        irmp_store_bit(1, state);
                        wait_for_space = 0;
                    } else if (irmp_pulse_time >= state->irmp_param.pulse_0_len_min &&
                               irmp_pulse_time <= state->irmp_param.pulse_0_len_max &&
                               irmp_pause_time >= state->irmp_param.pause_0_len_min && irmp_pause_time <=
                                                                                               state->irmp_param.pause_0_len_max) {                                                               // pulse & pause timings correct for "0"?
#ifdef ANALYZE
                        ANALYZE_PUTCHAR ('0');
                        ANALYZE_NEWLINE ();
#endif // ANALYZE
            ...
            if (irmp_start_bit_detected && state->irmp_bit == state->irmp_param.complete_len &&
                    state->irmp_param.stop_bit == 0)    // enough bits received?
            {
                if (last_irmp_command == state->irmp_tmp_command && key_repetition_len < AUTO_FRAME_REPETITION_LEN) {
                ...
                if (state->irmp_param.protocol == IRMP_SIRCS_PROTOCOL && (repetition_frame_number == 1 || repetition_frame_number == 2))
                {
#ifdef ANALYZE
...

                {
...
#endif // ANALYZE
                    state->irmp_ir_detected = TRUE;

#if IRMP_SUPPORT_DENON_PROTOCOL == 1
                    if (state->irmp_param.protocol == IRMP_DENON_PROTOCOL)
                    {                                                               // check for repetition frame
                        if ((~state->irmp_tmp_command & 0x3FF) == last_irmp_denon_command) // command bits must be inverted
                        {
                            state->irmp_tmp_command = last_irmp_denon_command;             // use command received before!
                            last_irmp_denon_command = 0;

                            state->irmp_protocol = state->irmp_param.protocol;                    // store protocol
                            state->irmp_address = state->irmp_tmp_address;                        // store address
                            state->irmp_command = state->irmp_tmp_command;                        // store command
                        }
                        else
                        {
                            if ((state->irmp_tmp_command & 0x01) == 0x00)
                            {
#ifdef ANALYZE
                                ANALYZE_PRINTF ("%8.3fms info Denon: waiting for inverted command repetition\n", (double) (time_counter * 1000) / F_INTERRUPTS);
#endif // ANALYZE
                                last_irmp_denon_command = state->irmp_tmp_command;
                    ...
                    if (state->irmp_param.protocol == IRMP_GRUNDIG_PROTOCOL && state->irmp_tmp_command == 0x01ff)
                    {                                                               // Grundig start frame?
#ifdef ANALYZE
                    ...
                    if (state->irmp_param.protocol == IRMP_NOKIA_PROTOCOL && state->irmp_tmp_address == 0x00ff && state->irmp_tmp_command == 0x00fe)
                    {                                                               // Nokia start frame?
#ifdef ANALYZE
                    ...
                    {
#if IRMP_SUPPORT_NEC_PROTOCOL == 1
                        if (state->irmp_param.protocol == IRMP_NEC_PROTOCOL && state->irmp_bit == 0)  // repetition frame
                        {
                            if (key_repetition_len < NEC_FRAME_REPEAT_PAUSE_LEN_MAX)
                            {
#ifdef ANALYZE
                        ...
                        if (state->irmp_param.protocol == IRMP_KASEIKYO_PROTOCOL)
                        {
                            ...
                            xor_value = (state->xor_check[0] & 0x0F) ^ ((state->xor_check[0] & 0xF0) >> 4) ^ (state->xor_check[1] & 0x0F) ^ ((state->xor_check[1] & 0xF0) >> 4);

                            if (xor_value != (state->xor_check[2] & 0x0F))
                            {
#ifdef ANALYZE
                                ANALYZE_PRINTF ("error 4: wrong XOR check for customer id: 0x%1x 0x%1x\n", xor_value, state->xor_check[2] & 0x0F);
#endif // ANALYZE
                                state->irmp_ir_detected = FALSE;
                            }

                            xor_value = state->xor_check[2] ^ state->xor_check[3] ^ state->xor_check[4];

                            if (xor_value != state->xor_check[5])
                            {
#ifdef ANALYZE
                                ANALYZE_PRINTF ("error 5: wrong XOR check for data bits: 0x%02x 0x%02x\n", xor_value, state->xor_check[5]);
#endif // ANALYZE
                                state->irmp_ir_detected = FALSE;
                            }

                            state->irmp_flags |= state->genre2;       // write the state->genre2 bits into MSB of the flag byte
                        }
#endif // IRMP_SUPPORT_KASEIKYO_PROTOCOL == 1
                        ...
                        if (irmp_param.protocol == IRMP_ORTEK_PROTOCOL)
                        {
                            ...
                            if ((state->irmp_tmp_address & 0x03) == 0x02)
                            {
#ifdef ANALYZE
                        ...
                        if (irmp_param.protocol == IRMP_MITSU_HEAVY_PROTOCOL)
                        {
                            check = state->irmp_tmp_command >> 8;                    // inverted upper byte == lower byte?
                            check = ~ check;
                            if (check == (state->irmp_tmp_command & 0xFF)) {         //ok:
                              state->irmp_tmp_command &= 0xFF;
                            }
...
#if IRMP_SUPPORT_RC6_PROTOCOL == 1
                        if (state->irmp_param.protocol == IRMP_RC6_PROTOCOL && state->irmp_param.complete_len == RC6_COMPLETE_DATA_LEN_LONG)     // RC6 mode = 6?
                        {
                            state->irmp_protocol = IRMP_RC6A_PROTOCOL;
                        }
                        else
#endif // IRMP_SUPPORT_RC6_PROTOCOL == 1
                        {
                            state->irmp_protocol = state->irmp_param.protocol;
                        }

#if IRMP_SUPPORT_FDC_PROTOCOL == 1
                        if (irmp_param.protocol == IRMP_FDC_PROTOCOL)
                        {
                            if (state->irmp_tmp_command & 0x000F)                          // released key?
                            {
                                state->irmp_tmp_command = (state->irmp_tmp_command >> 4) | 0x80;  // yes, set bit 7
                            }
                            else
                            {
                                state->irmp_tmp_command >>= 4;                             // no, it's a pressed key
                            }
                            state->irmp_tmp_command |= (state->irmp_tmp_address << 2) & 0x0F00;   // 000000CCCCAAAAAA -> 0000CCCC00000000
                            state->irmp_tmp_address &= 0x003F;
                        }
#endif

                        state->irmp_address = state->irmp_tmp_address;                            // store address
#if IRMP_SUPPORT_NEC_PROTOCOL == 1
                        if (state->irmp_param.protocol == IRMP_NEC_PROTOCOL)
                        {
                            last_irmp_address = state->irmp_tmp_address;                   // store as last address, too
                        }
#endif

#if IRMP_SUPPORT_RC5_PROTOCOL == 1
                        if (state->irmp_param.protocol == IRMP_RC5_PROTOCOL)
                        {
                            state->irmp_tmp_command |= rc5_cmd_bit6;                       // store bit 6
                        }
#endif
#if IRMP_SUPPORT_S100_PROTOCOL == 1
...
#if IRMP_SUPPORT_SAMSUNG_PROTOCOL == 1
                        state->irmp_id = state->irmp_tmp_id;
#endif
                    }
                }

                if (state->irmp_ir_detected) {
                    if (last_irmp_command == state->irmp_tmp_command &&
                        last_irmp_address == state->irmp_tmp_address &&
                        key_repetition_len < IRMP_KEY_REPETITION_LEN) {
                        state->irmp_flags |= IRMP_FLAG_REPETITION;
                    }

                    last_irmp_address = state->irmp_tmp_address;                           // store as last address, too
                    last_irmp_command = state->irmp_tmp_command;                           // store as last command, too
...
#if IRMP_SUPPORT_JVC_PROTOCOL == 1
                if (state->irmp_protocol == IRMP_JVC_PROTOCOL)                             // the stop bit of JVC frame is also start bit of next frame
                {                                                                   // set pulse time here!
